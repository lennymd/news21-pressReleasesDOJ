	...
	if path_to_outfile.is_file(): 
		...
		pass
			...
			try: 
				...
				#All the title divs
				titles = page_of_interest.find_all('div', class_="views-field views-field-title")
				#print(len(titles))
				...
				for t in range(0, len(titles)):
					...
					tdate = title.find_previous('h3').find('span', class_="date-display-single").attrs['content']
